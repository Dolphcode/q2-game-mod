	...
	*/
	...
	else if (!Q_stricmp(s, ent->client->pers.weapon->pickup_name) || !Q_stricmp(s, "Blaster"))
	{
		...
		if (flag) {
			...
			else if (ent->client->pers.inventory[ITEM_INDEX(FindItem("Torch"))] && ent->client->pers.inventory[ITEM_INDEX(FindItem("TorchUses"))] && flag < 0b1000) {
				it = FindItem("Torch");
			}
			else if (ent->client->pers.inventory[ITEM_INDEX(FindItem("LuxuryPickaxe"))] && ent->client->pers.inventory[ITEM_INDEX(FindItem("LuxuryPickaxeUses"))] && flag < 0b10000) {
				it = FindItem("LuxuryPickaxe");
			}
			else if (ent->client->pers.inventory[ITEM_INDEX(FindItem("Shovel"))] && ent->client->pers.inventory[ITEM_INDEX(FindItem("ShovelUses"))] && flag < 0b100000) {
				it = FindItem("Shovel");
			}
...
void Cmd_Craft_f(edict_t* ent) {
	...
	if (!Q_stricmp(gi.args(), "pick")) {
		...
		gi.cprintf(ent, PRINT_HIGH, "No such item %s", gi.args());
	}
}
