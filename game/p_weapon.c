...
void fire_pickaxe(edict_t* self, vec3_t start, vec3_t aimdir, int damage, int kick) {
	...
	if (!(tr.fraction < 1.0))
	{
		...
		VectorMA(start, PICKAXE_RANGE, forward, end);
		...
		if (!((tr.surface) && (tr.surface->flags & SURF_SKY)))
		{
			if (tr.fraction < 1.0)
			{
				if (!Q_stricmp(tr.ent->classname, "mineable"))
				{
...
void pickaxe_attack(edict_t* ent, vec3_t g_offset, int damage) {
	...
	*/
	...
	fire_pickaxe(ent, start, forward, damage, PICKAXE_KICK);
...
void Weapon_Pickaxe_Fire(edict_t* ent) {
	int damage;
	if (deathmatch->value)
		damage = PICKAXE_DEATHMATCH_DAMAGE;
	else
	...
	pickaxe_attack(ent, vec3_origin, damage);
	ent->client->ps.gunframe++;
}
...
void fire_axe(edict_t* self, vec3_t start, vec3_t aimdir, int damage, int kick) {
	...
	if (!(tr.fraction < 1.0))
	{
		vectoangles(aimdir, dir);
		AngleVectors(dir, forward, right, up);
		VectorMA(start, AXE_RANGE, forward, end);
		...
		if (!((tr.surface) && (tr.surface->flags & SURF_SKY)))
		{
			if (tr.fraction < 1.0)
			{
				if (!Q_stricmp(tr.ent->classname, "choppable"))
				{
					T_Damage(tr.ent, self, self, aimdir, tr.endpos, tr.plane.normal, damage, kick, DAMAGE_BULLET, 0);
					gi.WriteByte(svc_temp_entity);
					gi.WriteByte(TE_SPARKS);
...
void Weapon_Torch(edict_t* ent)
{
	...
	Weapon_Generic(ent, 5, 16, 59, 64, pause_frames, fire_frames, Weapon_Torch_Fire);
}
...
void fire_luxpickaxe(edict_t* self, vec3_t start, vec3_t aimdir, int damage, int kick) {
	...
	if (!(tr.fraction < 1.0))
	{
		vectoangles(aimdir, dir);
		AngleVectors(dir, forward, right, up);
		VectorMA(start, LUXPICKAXE_RANGE, forward, end);
		...
		if (!((tr.surface) && (tr.surface->flags & SURF_SKY)))
		{
			if (tr.fraction < 1.0)
			{
				if (!Q_stricmp(tr.ent->classname, "mineable") || !Q_stricmp(tr.ent->classname, "luxmineable"))
				{
					T_Damage(tr.ent, self, self, aimdir, tr.endpos, tr.plane.normal, damage, kick, DAMAGE_BULLET, 0);
					gi.WriteByte(svc_temp_entity);
					gi.WriteByte(TE_SPARKS);
...
void luxpickaxe_attack(edict_t* ent, vec3_t g_offset, int damage) {
	...
	*/
	...
	fire_luxpickaxe(ent, start, forward, damage, LUXPICKAXE_KICK);
...
void Weapon_LuxPickaxe_Fire(edict_t* ent) {
	int damage;
	if (deathmatch->value)
		damage = LUXPICKAXE_DEATHMATCH_DAMAGE;
	else
		damage = LUXPICKAXE_NORMAL_DAMAGE;
	...
	luxpickaxe_attack(ent, vec3_origin, damage);
	ent->client->ps.gunframe++;
}
...
void Weapon_LuxPickaxe(edict_t* ent) {
	...
	Weapon_Generic (ent, 5, 20, 49, 53, pause_frames, fire_frames, Weapon_LuxPickaxe_Fire);
}
...
void fire_shovel(edict_t* self, vec3_t start, vec3_t aimdir, int damage, int kick) {
	...
	if (!(tr.fraction < 1.0))
	{
		...
		if (!((tr.surface) && (tr.surface->flags & SURF_SKY)))
		{
			if (tr.fraction < 1.0)
			{
				if (!Q_stricmp(tr.ent->classname, "diggable"))
				{
					T_Damage(tr.ent, self, self, aimdir, tr.endpos, tr.plane.normal, damage, kick, DAMAGE_BULLET, 0);
					gi.WriteByte(svc_temp_entity);
					gi.WriteByte(TE_SPARKS);
...
void shovel_attack(edict_t* ent, vec3_t g_offset, int damage) {
	...
	*/
	...
	fire_shovel(ent, start, forward, damage, SHOVEL_KICK);
...
void Weapon_Shovel_Fire(edict_t* ent) {
	int damage;
	if (deathmatch->value)
		damage = SHOVEL_DEATHMATCH_DAMAGE;
	else
		damage = SHOVEL_NORMAL_DAMAGE;
	...
	shovel_attack(ent, vec3_origin, damage);
	ent->client->ps.gunframe++;
}
...
void Weapon_Shovel(edict_t* ent) {
	...
	Weapon_Generic(ent, 6, 17, 57, 61, pause_frames, fire_frames, Weapon_Shovel_Fire);
}
...
void Weapon_Barbel_Fire(edict_t* ent) {
	if (ent->mightiness < 100 && ent->client->pers.inventory[ent->client->ammo_index]) {
		if (!((int)dmflags->value & DF_INFINITE_AMMO))
			ent->client->pers.inventory[ent->client->ammo_index]--;
...
void Weapon_Barbel(edict_t* ent) {
	...
	Weapon_Generic(ent, 7, 18, 36, 39, pause_frames, fire_frames, Weapon_Barbel_Fire);
}